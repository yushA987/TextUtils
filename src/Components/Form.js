...
export default function Form(props) {
    ...
    const AddComa = () =>{
        ...
        if(newStr == text){
            props.showAlert("Coma's already added", "Failed")
        }
        else{
            setText(newStr);
            props.showAlert("Coma's Added", "Success");
        }
    }
    return (
    <>
        <div className="container my-0" style={{color: props.mode==='dark'?'white':'black'}}>
            ...
            <button disabled={text.length===0} className="btn btn-primary my-3 mx-2" onClick={AddComa}>Add Comas</button>
        </div>
